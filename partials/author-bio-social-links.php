...
<ul class="social-links">
	<?php if ( $fb = $author_obj->fb ) { ?>
		<li class="facebook">
			<a href="https://facebook.com/<?php echo largo_fb_url_to_username(esc_url( $fb )); ?>" title="Follow <?php echo esc_attr( $author_obj->display_name ); ?> on Facebook" rel="me"><i class="icon-facebook"></i></a>
		</li>
	<?php } ?>

	<?php if ( $twitter = $author_obj->twitter ) { ?>
		<li class="twitter">
			<a href="https://twitter.com/<?php echo largo_twitter_url_to_username ( $twitter ); ?>" title="Follow <?php echo esc_attr( largo_twitter_url_to_username ( $twitter ) ); ?> on Twitter" ><i class="icon-twitter"></i></a>
		</li>
	<?php } ?>

	<?php if ( $email && $show_email !== 'off' ) { ?>
		<li class="email">
			<a class="email" href="mailto:<?php echo esc_attr( $email ); ?>" title="Email <?php echo esc_attr( $author_obj->display_name ); ?>"><i class="icon-mail"></i></a>
		</li>
	<?php } ?>

	<?php if ( $linkedin = $author_obj->linkedin ) { ?>
		<li class="linkedin">
			<a href="<?php echo esc_url( $linkedin ); ?>" title="Follow <?php echo esc_attr( $author_obj->display_name ); ?> on LinkedIn"><i class="icon-linkedin"></i></a>
		</li>
	<?php } ?>
	...
	<?php
		if ( !is_author() ) {
			printf(
				// translators: %1$s is the author archive URL. %2$s is the author's first name.
